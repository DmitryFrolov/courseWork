...
{
	chessBoard = new ChessBoard(Settings::getInstance().getAmountOfCellsInARow(), 
		Settings::getInstance().getCellMaxWeight());
	chessBoard->create();
...
{
	auto scale = Director::getInstance()->getVisibleSize().height /
		(250 * Settings::getInstance().getAmountOfCellsInARow());
	playableFigure = new Figure(cells.at(0).at(0)->centerCoordinate, scale, Vec2(0, 0));
...
void PlayGameScene::drawPlayerScoreLabels()
{
	...
	player2ScoreLabel = Label::createWithTTF("Player 2 score: " + std::to_string(player2Score),
		"fonts/Enchanted Land cyr-lat.ttf", visibleSize.height / 14);
	player2ScoreLabel->setPosition(Vec2(origin.x + visibleSize.width - 
		player2ScoreLabel->getBoundingBox().size.width / 1.3,
		origin.y + visibleSize.height - 
		player2ScoreLabel->getBoundingBox().size.height ));
...
void PlayGameScene::drawPauseButton()
{
	...
	auto closeItem = MenuItemImage::create("settings-icon.png", "settings-icon.png", 
		CC_CALLBACK_1(PlayGameScene::showDialogPause, this));
...
void PlayGameScene::onMouseUp(Event *event)
{
		...
		if ((mEvent->getMouseButton() == MOUSE_BUTTON_LEFT) 
			&& chessBoard->isPositionBelongsToBoard(cursorPosition)
			&& playableFigure->getFigureReleased()
			&& cells.at(chessBoard->ConvertVec2toVec2B(cursorPosition).x).
			at(chessBoard->ConvertVec2toVec2B(cursorPosition).y)->scoreWeight != 0 
			&& playableFigure->isTargetCoordsValid(chessBoard->ConvertVec2toVec2B(cursorPosition), 
				playableFigure->getFigureOnBoard()))
		{ 
...
void PlayGameScene::runMainGameSequence(Sprite *obj, Vec2 moveto)
{
	...
	auto releaseHorse = CallFunc::create([this]() {
		playableFigure->setFigureReleased(true); 
	});
	auto upgradeScore = CallFunc::create([this]() {
		auto currentCell = cells.at(playableFigure->getFigureOnBoard().x).
			at(playableFigure->getFigureOnBoard().y);
	...
	if(SettingsConfRW::readAIEnabled())
		sequence = Sequence::create(acMoveTo, upgradeScore, checkAvailableCellsExist,
			switchTurn, DelayTime::create(0.5), aiTurn, DelayTime::create(ANIMATION_LENGHT),
			upgradeScore, checkAvailableCellsExist, switchTurn, releaseHorse, nullptr);
	else
		sequence = Sequence::create(acMoveTo, upgradeScore, checkAvailableCellsExist, 
			switchTurn, releaseHorse, nullptr);
...
void PlayGameScene::aiPlay()
{
	...
	for (size_t iteratorAvailable = 0; iteratorAvailable < available_cells->size(); ++iteratorAvailable)
	{
		for (auto &row : cells)									//check enemy available moves
			for (auto &cell : row)
				if (playableFigure->isTargetCoordsValid(cell->coordinatesInTab, 
					available_cells->at(iteratorAvailable).coordinatesInTab)			
					&& cell->coordinatesInTab != playableFigure->getFigureOnBoard())//and target position != current figure position
					if (available_cells->at(iteratorAvailable).scoreWeight != 0 &&
						cell->scoreWeight != 0) {	//not plot a route through 0-value cells
...
std::vector<Cell> *PlayGameScene::getAvailableCells()
{
	...
	for (auto &row : cells)
		for (auto &cell : row)
		{
			if (cell->scoreWeight > 0 && playableFigure->isTargetCoordsValid(cell->coordinatesInTab,
				playableFigure->getFigureOnBoard())) {
